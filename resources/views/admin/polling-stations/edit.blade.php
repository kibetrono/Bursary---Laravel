...
@section('title')
    Update Polling Station: {{ $pollingStation->name }}
@endsection
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-info">
                            <div class="card-header">
                                <h3 class="card-title">Update Polling Station: {{ $pollingStation->name }}</h3>
                                <div class="card-tools">
                                </div>
                            </div>
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form action="{{ route('polling-station.update', $pollingStation->id) }}" method="POST" class="my-4">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    ...
                                    <div class="input-group mb-3 px-4">
                                        <select id="sublocation_name" class="form-control @error('sublocation_id') is-invalid @enderror"
                                            ...
                                            @foreach ($subLocations as $subLocations)
                                                <option value="{{ $subLocations->id }}"
                                                    {{ $subLocations->id == $pollingStation->sublocation_id ? 'selected' : '' }}>
                                                    {{ $subLocations->name }}
                                                </option>
                                            @endforeach
                                    ...
                                    <div class="input-group mb-3 px-4">
                                        <input id="name" type="text"
                                            ...
                                            value="{{ $pollingStation->name }}" required autocomplete="name" autofocus
                                    ...
                                    <div class="px-4">
                                        <button style="border:unset" type="submit"
                                            class="btn-md btn btn-success btn-block"><i class="fas fa-edit"></i> Update Polling Station</button>
                                    </div>
